...
$("#editFeminineForm").validate({
    onkeyup: function (element) {
        // Validate email and contact number fields on keyup
        if ($(element).attr('id') === 'edit_email_address') {
            $(element).valid(); // Validate email
        }

        if ($(element).attr('id') === 'edit_contact_no') {
            $(element).valid(); // Validate contact number
        }
    },
    rules: {
        edit_first_name: {
        ...
        edit_contact_no: {
            required: false, // Make contact number optional
            digits: true,
            minlength: 10,
            maxlength: 11
        }
    ...
    messages: {
        ...
        edit_email_address: {
            email: "Please enter a valid email address", // Message if email format is incorrect
        },
        edit_menstruation_status: {
            required: "Please select the current menstruation status of the user",
        },
